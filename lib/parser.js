...
Parser.audio = function (req, res, next) {
    console.log('Audio fetch request seen!');
...
Parser.hangup = function(req, res, next) {     
    req.addListener('data', function(payload){
        ...
        if(payload['event'] == "hangup") {
            if(payload['extras'] && payload['extras']['answerTime'] && payload['extras']['releaseTime']) {
                var duration = payload['extras']['answerTime'] - payload['extras']['releaseTime'];
                if(duration > 3) {
                	collection.find().count(function (e, count) {
                    	var data = collection.find({id: count});
		...
		/*var post_options = {
		  host: 'guard.cloudapp.net',
		  port: '80',
		  path: '/askfast/callserver?event=hangup&user_id=x31642244517_0&sequenceNumber=null',
		  method: 'POST',
		  headers: {
        ...
        //redirect to TIZIN proxy
        var post_req = http.request(post_options, function(response) {
        	response.setEncoding('utf8');
	        return response.on('data', function (responseFromTIZIN) {
	        	console.log('Response: ' + responseFromTIZIN);
	        	res.pipe(responseFromTIZIN);
	        	res.end();
        		return res;
	        });
	    });
		...
		request.post('http://guard.cloudapp.net/askfast/callserver?event=hangup&user_id=x31642244517_0&sequenceNumber=null', {data: payload},function(err, response, body) {
		}).pipe(res);
    });
}
