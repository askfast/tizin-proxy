...
Parser.audio = function (req, res, next) {
    ...
    res.writeHead(302, {
        ...
        'Location': '/audio/alarm_nood_hekje.wav'
    });
    res.end();
    return res;
}
...
Parser.question = function(req, res, next) {     
    ...
    var sessionKey = req.query.sessionKey;     
	log('Question request from: '+ req.connection.remoteAddress + ' Proxy: ' + req.headers['X-Forwarded-For'] + " sessionKey: "+ sessionKey);
    if(requester && responder && remoteHostNameCheckOn == req.connection.remoteAddress) {
        log('Requester: ' + requester + ' and Responder: '+ responder +'found! ');
        //put the timestamp in the db
        collection.find().count(function (e, count) {
            var document = { questionFetchTimestamp: new Date(), id: count + 1, audioFetchTimestamp: null, 
            	sessionId: sessionKey};
            log('Adding new document: ' + document);
            collection.insert(document, {w: 1}, function(err, records){
                log("Record added as "+records[0]._id);
            });
        });
    }     
    ...
    req.addListener('data', function(payload){
        ...
        if(jsonPayload['event'] == "hangup" && remoteHostNameCheckOn == req.connection.remoteAddress) {
            if(jsonPayload['extras'] && jsonPayload['extras']['answerTime'] && jsonPayload['extras']['releaseTime']) {
            	...
            	var sessionKey = jsonPayload['extras']['sessionKey'];
            	log('hangup for sessionKey: '+ sessionKey);
                if(duration > 1000) {
                	collection.find().count(function (e, count) {
                    	collection.findOne({sessionId: sessionKey}, function(err, document) {
                    		if(document && (!document.audioFetchTimestamp || !document.audioFetchTimestamp.getTime())) {
                    			log("======No audio fetched for id: "+ document.id + "=============");
                    		}
                    	});
                    });
                }
