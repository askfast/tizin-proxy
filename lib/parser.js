...
Parser.audio = function (req, res, next) {
    log('Audio fetch request seen from: '+ req.connection.remoteAddress);
...
Parser.question = function(req, res, next) {     
	...
	log('Question request from: '+ req.connection.remoteAddress + ' Proxy: ' + req.headers['X-Forwarded-For']);
